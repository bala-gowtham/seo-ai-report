...
if (
new TextEncoder().encode(rawBody).length > 80_000
) {
return jsonResponse(
{
ok: false,
...
error:
"The AI service authentication is not configured.",
requestId,
retryable: false,
},
500,
...
if (
  upstreamResponse.status === 202 &&
  payload?.pending === true &&
  payload?.code === "REPORT_REQUIRED"
) {
  return jsonResponse(
    {
      ...payload,
  ...
  return jsonResponse(
    {
      ok: false,
      code:
        payload?.code || "AI_UPSTREAM_ERROR",
      error:
        payload?.error ||
        payload?.message ||
